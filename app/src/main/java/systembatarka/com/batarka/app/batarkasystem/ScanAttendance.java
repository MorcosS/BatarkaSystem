...
public class ScanAttendance extends AppCompatActivity implements ZXingScannerView.ResultHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(day==Calendar.FRIDAY) {
            editor.putString("LastWeekDate", c.getTime().getDate() + "-" + c.getTime().getMonth() + "-" + (c.getTime().getYear() + 1900));
            editor.commit();
        }
       ...
       DatabaseReference connectedRef = FirebaseDatabase.getInstance().getReference(".info/connected");
        connectedRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot snapshot) {
                ...
                if (connected) {
                    Cursor cursor;
                    cursor = dbHelperScanedIDs.getOrder();
                    try {
...
query.addValueEventListener(new com.firebase.client.ValueEventListener() {
    ...
    public void onDataChange(com.firebase.client.DataSnapshot dataSnapshot) {
        if(!dataSnapshot.child(c.getTime().getDate() + "-" + c.getTime().getMonth() + "-" +  (c.getTime().getYear() + 1900)).exists()){
            for (int i = 0; i < MainActivity.PlaceholderFragment.kashfFaslList.size(); i++) {
                MainActivity.myFirebaseRef.child("Classes").child(MainActivity.myClass).child("Attendance").child(c.getTime().getDate() + "-" + c.getTime().getMonth() + "-" + (c.getTime().getYear() + 1900)).child(MainActivity.PlaceholderFragment.kashfFaslList.get(i).getM5dumID()).setValue(false);
            }
        }
    }
                    ...
                    public void onClick(DialogInterface dialog, int id) {
                       ...
                       if(!sharedPref.getString("LastWeekDate","").equals("")){
                            ...
                            MainActivity.myFirebaseRef.child("Classes").child(MainActivity.myClass).child("Attendance").child(sharedPref.getString("LastWeekDate","")).child(MainActivity.PlaceholderFragment.kashfFaslList.get(Integer.parseInt(seletedItems.get(i).toString())).getM5dumID()).setValue(true);
                                dbHelperScanedIDs.addOrder(Integer.parseInt(MainActivity.PlaceholderFragment.kashfFaslList.get(Integer.parseInt(seletedItems.get(i).toString())).getM5dumID()));
                        }
                        seletedItems.clear();
                       }
    ...
    public void handleResult(Result rawResult) {
       ...
       /* AlertDialog.Builder builder = new AlertDialog.Builder(this);
        ...
        alert1.show();*/
            String myOwnClass = checkThisID(rawResult.getText());
        ...
        if(!myOwnClass.equals("Not Found")) {
            if(!sharedPref.getString("LastWeekDate","").equals("")) {
               ...
               dbHelperScanedIDs.addOrder(Integer.parseInt(rawResult.getText().toString()));
